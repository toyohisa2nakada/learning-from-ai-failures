  ...
  stages: [
    {
      ...
      quiz: {
        title: "問題", problems: [
          ...
          { question: "教師データが「折れ線」のとき、個別グラフが1つだけのときにはすべての点を通ることができません。その理由として正しいものは？", choices: ["折れ線には上がる部分と下がる部分があるが、個別グラフは単調な動き（上がるだけ、または下がるだけ）しかできないから", "必ず原点(x=0, y=0)を通るため、折れ線を表現できないから"], correctIndex: 0 },
          { question: "教師データが原点を通らない場合、バイアスを0に固定すると、すべての点を通ることができません。その理由として正しいものは？", choices: ["バイアスが0の場合、折れ線を表現することができないため、原点を通らないグラフを表現できない", "バイアスが0の場合、加算結果は必ず原点(x=0,y=0)を通るため、原点を通らないグラフを表現できない"], correctIndex: 1 },
          { question: "ニューラルネットワークの構造で、ニューロン（ノード）を1つ増やして計3個にすると、画面上のグラフはどう変わりますか？", choices: ["グラフ1のパラメータが1つ増える", "グラフ2のパラメータが1つ増える", "個別のグラフが1つ増える", "グラフの加算結果の数が1つ増える"], correctIndex: 2 },
        ]
      },
    },
    {
      ...
      guide: [
        ...
        },
        {
          element: '#mse-value', popover: {
            title: '学習の評価',
            description: '誤差が0に近づくと学習が成功し、ここが緑に変わります。'
          }
        },
      ],
      quiz: {
        title: "問題", problems: [
          ...
          { question: "バイアスは使用していますか？", choices: ["使用している", "使用していない"], correctIndex: 0 },
          { question: "教師データを「原点を通る直線」としたとき、ニューロン数2、バイアスありで学習は成功しましたか？", choices: ["成功した", "失敗した"], correctIndex: 0 },
          {
            question: "学習に失敗するパターンをすべて選んでください。", choices: [
              "原点通る直線, ニューロン数1, バイアスなし",
              "原点通る折れ線, ニューロン数1, バイアスあり",
              "原点通らない直線, ニューロン数2, バイアスなし",
              "原点通らない折れ線, ニューロン数2, バイアスなし",
            ], correctIndex: [1, 2, 3]
          },
          {
            question: "ニューロン数2、バイアスありで 原点を通る折れ線 は理論的には学習可能です。しかし実際に実行してみると、まれに失敗することがあります。その理由として妥当なものをすべて選んでください。", choices: [
              "重みの初期値によって2つの個別グラフが同じ動きをしてしまうことがあるから",
              "学習率が小さいため、計算結果が途中で四捨五入されてしまうから",
              "入力データの順番が、常に小さい順に並んでいないから",
              "学習回数(epochs)が足りないから"
            ], correctIndex: [0, 3]
          },
        ]
...
const SCRIPT_BASE_PATH = '/chapter/fundamentals/';
          ...
          <div className="left-panel flex flex-col overflow-hidden min-h-0" style={{ width: `${leftWidth}%`, flexShrink: 0 }}>
            ...
            <div id="drawing-area" className="flex-1 min-h-0 flex flex-col">
              ...
              {programmingMode === 'manual' ? <NeuralNetGraph /> :
                <JsEditor
                  path="chapter/fundamentals/main.js"
