...
export class MultiHeadAttention extends tf.layers.Layer {
    ...
    build(inputShape) {
        ...
        // console.log("this.kernelK.shape", this.kernelK.shape)
        if (this.useBias) {
            this.biasK = this.addWeight(
                'bias_k',
                // 前がオリジナルのバイアス、後者がキーのバイアスを拡張した独自バージョン
                this.type === "nor" ? [this.numHeads * this.keyDim] : [queryShape[queryShape.length - 2], this.numHeads * this.keyDim],
                'float32',
                tf.initializers.zeros(),
                null,
                true
